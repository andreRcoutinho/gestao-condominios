...
const UNIT_IMPORT_REQUEST: String = "Importar frações";
const UNIT_IMPORT_MESSAGE_SUCCESS: String = "Frações importadas com sucesso!";
const UNIT_IMPORT_MESSAGE_FAIL: String = "Ocorreu um erro ao importar as frações";
...
export async function importUnits(req: Request, res: Response) {

    var response = await unitService.importUnits(req.body);

    if (response instanceof Error) {
        return res.status(HttpStatus.BAD_REQUEST).send(new ApiResponse(UNIT_IMPORT_REQUEST, UNIT_IMPORT_MESSAGE_FAIL, HttpStatus.BAD_REQUEST, {}, response.message));
    } else {
        return res.send(new ApiResponse(UNIT_IMPORT_REQUEST, UNIT_IMPORT_MESSAGE_SUCCESS, HttpStatus.OK, response))
    }
}