...
async function createNormalPaymentMap(
	units: Unit[],
	total_value: Number,
	...
	try {
		...
		if (yearly) {
			let payment_map_values: PaymentMapValues = new PaymentMapValues(
				body.value,
				new Date(new Date().getFullYear() + '-01'),
				payment_map,
				body.value * 0.1
			);
...
function calculateReserveFunds(
	units: Unit[],
	total_permilage: number,
	total_value: number
): { id; reserve_fund }[] {
	let reserve_funds: { id; reserve_fund }[] = [];
	for (let i = 0; i < units.length; i++) {
...
async function saveMap(
	reserve_funds: { id; reserve_fund }[],
	monthly_expenses: { id; monthy_expense }[],
	payment_map: PaymentMap,
	units: Unit[]
) {
